...
if [ -n "${ATUIN_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$ATUIN_INSTALLER_GHE_BASE_URL"
else
    INSTALLER_BASE_URL="${ATUIN_INSTALLER_GITHUB_BASE_URL:-https://github.com}"
fi
if [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URL="$INSTALLER_DOWNLOAD_URL"
else
    ARTIFACT_DOWNLOAD_URL="${INSTALLER_BASE_URL}/atuinsh/atuin/releases/download/v18.5.0"
fi
PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
if [ -n "${ATUIN_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$ATUIN_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${ATUIN_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${ATUIN_UNMANAGED_INSTALL:-}"
...
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.28.3"},"source":{"app_name":"atuin","name":"atuin","owner":"atuinsh","release_type":"github"},"version":"18.5.0"}
EORECEIPT
RECEIPT_HOME="${XDG_CONFIG_HOME:-$HOME/.config}/atuin"
...
The installer for atuin 18.5.0

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/atuinsh/atuin/releases/download/v18.5.0
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set ATUIN_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "atuin-aarch64-apple-darwin.tar.gz")
            ...
            _checksum_value="2c57ff6ce55106dac337a02e828667c19bcad5ff98cfa695414e7abfc85ce8de"
            _bins="atuin"
        ...
        "atuin-aarch64-unknown-linux-gnu.tar.gz")
            ...
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="fc0472bb5d5f57efe48a3188434035d44d6dac4a9fee3866b8ff4ede7ee1c4ef"
            _bins="atuin"
            _bins_js_array='"atuin"'
            _libs=""
            _libs_js_array=""
        ...
        "atuin-aarch64-unknown-linux-musl.tar.gz")
            ...
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="eaee559c1494be99e0043407c3cea5c79b28c359a5a9fa2d256d3f2842bde887"
            _bins="atuin"
            _bins_js_array='"atuin"'
            _libs=""
            _libs_js_array=""
        ...
        "atuin-x86_64-apple-darwin.tar.gz")
            ...
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="2ede961cdde053d73a46bdbc730831306dbc50c771a188eb68963194b7b910e5"
            _bins="atuin"
            _bins_js_array='"atuin"'
            _libs=""
            _libs_js_array=""
        ...
        "atuin-x86_64-unknown-linux-gnu.tar.gz")
            ...
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="00db1d5467ffb783ee1306a6755a13903932ef2f1c8661a73679633e8a961821"
            _bins="atuin"
            _bins_js_array='"atuin"'
            _libs=""
            _libs_js_array=""
        ...
        "atuin-x86_64-unknown-linux-musl.tar.gz")
            ...
            _zip_ext=".tar.gz"
            _checksum_style="sha256"
            _checksum_value="47a09d0b7e696b72b09fe37d5c1933ba22ee7bf131574493f66b1260aca2915d"
            _bins="atuin"
            _bins_js_array='"atuin"'
            _libs=""
            _libs_js_array=""
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="atuin-aarch64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="atuin-x86_64-apple-darwin.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="atuin-aarch64-unknown-linux-gnu.tar.gz"
            ...
            _archive="atuin-aarch64-unknown-linux-musl.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
        ...
        "x86_64-apple-darwin")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="atuin-x86_64-unknown-linux-gnu.tar.gz"
            if ! check_glibc "2" "35"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
            ...
            _archive="atuin-x86_64-unknown-linux-musl.tar.gz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
...
install() {
    ...
    if [ -n "${ATUIN_INSTALL_DIR:-}" ]; then
        _force_install_dir="$ATUIN_INSTALL_DIR"
    ...
    if [ -z "${_install_dir:-}" ]; then
        ...
        if [ -n "${HOME:-}" ]; then
            ...
            _lib_install_dir="$HOME/.atuin/bin"
            _receipt_install_dir="$_install_dir"
